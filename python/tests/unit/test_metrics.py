...
class TestPSNR:
    ...
    def test_solid_color_pair(self) -> None:
        """PSNR between solid red and solid blue images should be finite and low."""
        solid_red = np.zeros((64, 64, 3), dtype=np.float32)
        solid_red[:, :, 0] = 1.0  # R=1, G=0, B=0
        solid_blue = np.zeros((64, 64, 3), dtype=np.float32)
        solid_blue[:, :, 2] = 1.0  # R=0, G=0, B=1
        result = ImageMetrics.psnr(solid_red, solid_blue)
        assert np.isfinite(result)
        # MSE = (1^2 + 0 + 1^2) / 3 = 2/3 -> PSNR ~ 1.76 dB
        assert result < 5.0

    def test_black_vs_black(self) -> None:
        """PSNR of identical black images should be infinity."""
        black = np.zeros((32, 32, 3), dtype=np.float32)
        result = ImageMetrics.psnr(black, black)
        assert result == float("inf")
...
class TestSSIMMap:
    ...
    def test_values_between_zero_and_one(
        ...
        """SSIM map values should be in [0, 1] for non-negative images."""
        smap = ImageMetrics.ssim_map(reference_image, noisy_image)
        assert smap.min() >= 0.0
        assert smap.max() <= 1.0
...
class TestMSE:
    ...
    def test_always_non_negative(self) -> None:
        """MSE should always be non-negative, even for random image pairs."""
        rng = np.random.default_rng(123)
        for _ in range(10):
            a = rng.random((16, 16, 3)).astype(np.float32)
            b = rng.random((16, 16, 3)).astype(np.float32)
            result = ImageMetrics.mse(a, b)
            assert result >= 0.0
