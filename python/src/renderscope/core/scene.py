...
class SceneReference(BaseModel):
    ...
    ``renderer`` and ``samples`` nominate the ground truth for this scene:
    ``renderscope reference`` defaults to them so independently generated
    ...
    # A reference render is generated by RenderScope rather than published
    # upstream, so unlike scene geometry there is no third party to fetch it
    # from. Once references are hosted somewhere, setting these lets a
    # contributor download one instead of spending hours regenerating it.
    url: str | None = None
...
class SceneManager:
    ...
    def reference_target_path(self, scene_id: str) -> Path | None:
        """Where a scene's reference render *belongs*, whether or not it exists.

        :meth:`get_reference_path` answers "is there a reference to compare
        against" and returns ``None`` when the file is missing, which is the
        right answer for a reader but useless to a writer. Reference generation
        needs the destination, so producing a reference and consuming one agree
        on a single location instead of drifting apart.

        Returns ``None`` only when the manifest declares no reference for this
        scene, i.e. there is no canonical location to write to.
        """
        scene = self.get_scene(scene_id)
        if scene.reference is None:
            return None
        return self._scenes_dir / scene.reference.image
